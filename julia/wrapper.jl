function wrapper(G::NiceGraph, h0, α_st, α_fin, thrs::Thresh, h_ε, logSizes, logSteps, logTrack, logE, logΛ, inFun )
      while true
              (G.eps0 > 5.0) && break;

              if G.eps0 == h_ε
                      @printf "             alpha  stage started: \n";
                      G, thrs, track, p, Hs, Es, Λs, SZs, TrackS = alphaLevel(G, h0, α_st, α_fin, thrs, inFun; initial=true);
                      @printf " eps:  %f       ||||    functional :  %f  \n" G.eps0 track[end];
                      @printf "              alpha stage ended \n \n";
              else
                      G , thrs, track, p, Hs, Es, Λs, SZs, TrackS = alphaLevel(G, h0, α_st, α_fin, thrs, inFun); 
             ...
             (track[end] < 1e-5) && break; 
              ...
              ε_target = G.eps0 + h_ε; 
              ans, track, h_log, G, L1up, L1, L0, p, e_log, λ_log  = freeGradientTransition(ε_target, G, h0, thrs, p, inFun);
              G.e=ans;
